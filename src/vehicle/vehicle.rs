...
use scheduler::job::{JobPtr, HasJob};
use scheduler::job_scheduler::{NewTick, NewJob, TickComplete};
...
impl Vehicle {
	...
	fn get_current_pos(&self) -> Option<Coord> {
		self.current_task().and_then(|t| t.job_if_complete())
						   .and_then(|j| Some(j.end()))
	}
	
	fn new_job(&mut self, context: NewJob) -> () {
	...
	fn tick(&mut self, context: NewTick) -> TickComplete {
